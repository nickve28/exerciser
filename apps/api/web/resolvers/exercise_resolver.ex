defmodule Api.Resolvers.ExerciseResolver do
  ...
  def list(args, _info) do
    handle_result(Workout.Services.Exercise.list(args))
  end

  def get(%{id: id}, _info) do
    exercise = Workout.Services.Exercise.get(id)
    handle_result(exercise)
  end

  def get_categories(_args, _info) do
    handle_result(Workout.Services.Category.list)
  end

  def get(_, _), do: {:error, "No id specified"}

  def create(args, _info) do
    handle_result(Workout.Services.Exercise.create(args))
  end

  def update(args, _info) do
    handle_result(Workout.Services.Exercise.update(args))
  end

  def delete(%{id: id}, _info) do
    payload = %{id: String.to_integer(id)}
    handle_result(Workout.Services.Exercise.delete(payload))
  end

  def count(_, _) do
    handle_result(Workout.Services.Exercise.count)
  end
